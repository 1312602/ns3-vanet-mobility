...
static bool InitVehicle(Ptr<Highway> highway, int& VID)
{
	...
	Ptr<Vehicle> RSUdst=CreateRSU(highway, RSUdstPos, 3, true);
	Ptr<Vehicle> RSUsrc=CreateRSU(highway, RSUdstPos+RSUdistance, 3, false);

	// get RSUsrc broadcasting packet 1 at time=200
	unsigned int pID=1;
//	AddPacket(RSUsrc, pID);
	Simulator::Schedule(Seconds(200.0), &AddPacket, RSUsrc, pID);

	highway->AddVehicle(RSUsrc); highway->AddVehicle(RSUdst);

	cout << "\t\tCreated two RSUs [" << RSUdst->GetVehicleId() << RSUdst->GetCharDirection() << "] at " << RSUdstPos << " and [" << RSUsrc->GetVehicleId() << RSUsrc->GetCharDirection() << "] at " << RSUdstPos+RSUdistance << endl;
	...
	cout << "LOG " << nowtime.ns3::Time::GetSeconds() << " R " <<  RSUsrc->GetVehicleId() << ' ' <<  RSUdstPos+RSUdistance << '\n';

//	// For now just 1 vehicle on opposite lane
//	Ptr<Vehicle> vehicle1=CreateVehicle(highway, 1, -1);
...
{ 
	...
	// Defaults
	float simTime=400.0;		// 200s to fill 6km road
	int runNumber=1;
//	double transmissionPower=18.9; // not enough
	double transmissionPower=19.6; //
